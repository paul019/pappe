...
class Transformer:
    ...
    def analyze_and_offset_measurements(self, measurements: list[Measurement]):
        ...
        if min_x >= 0:
            offset_x = 0
            scale_x = (self.grid_config['num_x_blocks']-offset_x) / max_x
        elif max_x <= 0:
            offset_x = self.grid_config['num_x_blocks']
            scale_x = offset_x / (-min_x)
        else:
            offset_x = min_x/(min_x-max_x) * (
                self.grid_config['num_x_blocks']/self.grid_config['num_x_blocks_per_super_block'])
            if offset_x < self.grid_config['num_x_blocks']/self.grid_config['num_x_blocks_per_super_block']/2:
                offset_x = math.ceil(
                    offset_x) * self.grid_config['num_x_blocks_per_super_block']
            else:
                offset_x = math.floor(
                    offset_x) * self.grid_config['num_x_blocks_per_super_block']
            scale_x = min(offset_x / (-min_x),
                          (self.grid_config['num_x_blocks']-offset_x) / max_x)

        if min_y >= 0:
            offset_y = 0
            scale_y = (self.grid_config['num_y_blocks']-offset_y) / max_y
        elif max_y <= 0:
            offset_y = self.grid_config['num_y_blocks']
            scale_y = offset_y / (-min_y)
        else:
            offset_y = min_y/(min_y-max_y) * (
                self.grid_config['num_y_blocks']/self.grid_config['num_y_blocks_per_super_block'])
            if offset_y < self.grid_config['num_y_blocks']/self.grid_config['num_y_blocks_per_super_block']/2:
                offset_y = math.ceil(
                    offset_y) * self.grid_config['num_y_blocks_per_super_block']
            else:
                offset_y = math.floor(
                    offset_y) * self.grid_config['num_y_blocks_per_super_block']
            scale_y = min(offset_y / (-min_y),
                          (self.grid_config['num_y_blocks']-offset_y) / max_y)
        ...
        for factor in factors:
            ...
            if max_x * scale_x_2 + offset_x <= self.grid_config['num_x_blocks'] and min_x * scale_x_2 + offset_x >= 0:
                new_scale_x = scale_x_2

        for factor in factors:
            scale_y_2 = scale_y * factor
            if max_y * scale_y_2 + offset_y <= self.grid_config['num_y_blocks'] and min_y * scale_y_2 + offset_y >= 0:
